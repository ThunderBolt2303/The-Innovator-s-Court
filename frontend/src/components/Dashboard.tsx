...
export default function Dashboard() {
  ...
  if (!stats) return <div className="flex h-screen items-center justify-center text-primary animate-pulse text-2xl font-semibold">Initializing Innovator's Court Framework...</div>;
  ...
  // Prepare time-series data for the anomaly chart
    ...
    <div className="flex h-screen bg-background text-slate-200 overflow-hidden font-sans">
      ...
      <aside className="w-64 bg-card border-r border-slate-700/50 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          <NavItem icon={<ShieldCheck />} label="Threat Intelligence" />
          <NavItem icon={<Server />} label="System Nodes" />
          <NavItem icon={<AlertTriangle />} label="Incidents" />
          <NavItem icon={<Search />} label="Forensics" />
        </nav>
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
  return (
    <a href="#" className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all ${active ? 'bg-primary/10 text-primary border border-primary/20 shadow-sm shadow-primary/5' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}>
